...
class clusterToRoot(object):
	...
	def setupClusters(self):
		self.clusterStruct.true_id = ROOT.std.vector('int')()
		self.clusterStruct.tracker_module = ROOT.std.vector('int')()
		...
		self.tree.Branch('true.id', self.clusterStruct.true_id)
		self.tree.Branch('tracker.module', self.clusterStruct.tracker_module)
	...
	def StoreClusters(self, cldict):
		for k,entry in cldict.iteritems():
			...
			self.clusterStruct.true_id.clear()
			self.clusterStruct.tracker_module.clear()
			...
			for gg in entry:
				...
				self.clusterStruct.true_id.push_back(mi[1])
				self.clusterStruct.tracker_module.push_back(mi[2])
				self.clusterStruct.tracker_side.push_back(mi[3])
				self.clusterStruct.tracker_layer.push_back(mi[4])
				self.clusterStruct.tracker_column.push_back(mi[5])
				self.clusterStruct.geiger_x.push_back(gg.x)
				self.clusterStruct.geiger_y.push_back(gg.y)
				self.clusterStruct.geiger_z.push_back(gg.z)
				self.clusterStruct.geiger_sigma_z.push_back(gg.sigmaz)
				self.clusterStruct.geiger_r.push_back(gg.r)
				self.clusterStruct.geiger_sigma_r.push_back(gg.sigmar)
