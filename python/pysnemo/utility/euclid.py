...
class Vector3:
    ...
    def angle(self, other):
        """Return the angle to the vector other"""
        cosangle = self.dot(other) / (self.magnitude()*other.magnitude())
        if (math.fabs(cosangle)>=1.0):
                cosangle = math.copysign(1.0,cosangle)
...
class Matrix3:
    ...
    def __mul__(self, other):
        if isinstance(other, Matrix3):
            ...
            B = other
            P = Point3(0, 0, 0)
            P.x = A.a * B.x + A.b * B.y + A.c * B.z
            P.y = A.e * B.x + A.f * B.y + A.g * B.z
            P.z = A.i * B.x + A.j * B.y + A.k * B.z
            return P
